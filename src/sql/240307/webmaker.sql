...
DROP TABLE IF EXISTS `tb_modal_tmp`;
CREATE TABLE `tb_modal_tmp` (
  `md_code` int(11) NOT NULL DEFAULT 0 COMMENT 'md_type',
  `md_hd_title` varchar(50) DEFAULT NULL COMMENT '모달 헤드 타이틀',
  `md_title` varchar(255) DEFAULT NULL COMMENT '모달 타이틀',
  `md_main` mediumtext DEFAULT NULL COMMENT '모달 내용',
  `md_type` varchar(20) DEFAULT NULL COMMENT '모달 아이콘',
  PRIMARY KEY (`md_code`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='모달템플릿';

#
# Data for table "tb_modal_tmp"
#

INSERT INTO `tb_modal_tmp` VALUES (0,'경고','오류가 발생했습니다.',NULL,NULL),(1,'알림','개발자 모드를 사용중입니다.','본 플랫폼에서는 작가님들을 위한 저작권보호 정책으로 개발자모드를 차단하고있습니다. F12를 눌러 개발자모드를 꺼주시기 바랍니다.','alert'),(2,'알림','게시글을 등록했습니다.','요청하신 게시글 등록을 완료했습니다.','alert');
