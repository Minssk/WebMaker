...
<script>
	...
	function update_notice(){
		...
		var url = "<c:out value="${pageContext.request.contextPath}"/><c:out value="${rootUrl}"/>/BoardNoticeUpdate.do";
...
<div class="main_content">
			...
			<table>
				...
				<tbody>
					<tr>
						...
						<td><input id="title" class="form_input" name="noticeTitle" type="text" value="<c:out value="${resultMap.noticeTitle}"/>"></td>
					</tr>
					<tr>
						<th><label for="content">내용</label></th>
						<td><div id="content" class="form_input"><c:out value="${resultMap.noticeCn}" escapeXml="false"/></div></td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td class="sort_left">
							<input class="" type="radio" name="fixYn" id="chk1" value="N" <c:if test="${resultMap.fixYn eq 'N'}">checked</c:if>>
							<label class="me-3" for="chk1">
								고정 안함
							</label>
							
							<input class="" type="radio" name="fixYn" id="chk2" value="Y" <c:if test="${resultMap.fixYn eq 'Y'}">checked</c:if>>
	...
	<div class="button_wrap">
		<button class="btn_01" onclick="update_notice();">수정</button>
