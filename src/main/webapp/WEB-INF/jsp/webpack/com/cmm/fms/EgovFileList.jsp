		...
		<tr id="egov_file_view_table_tr_${status.count}">
			...
			<c:choose>
				<c:when test="${updateFlag eq 'Y'}">
					...
					<img src="<c:url value='/images/webpack/com/cmm/btn/btn_del.png' />" class="cursor" onClick="fn_egov_deleteFile('<c:out value="${fileVO.atchFileId}"/>','<c:out value="${fileVO.fileSn}"/>','egov_file_view_table_tr_${status.count}');" alt="<spring:message code="title.attachedFileDelete" />">
				</c:when>
				<c:otherwise>
