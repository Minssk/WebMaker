package webpack.com.sec.security.filter;
			...
			if (isRemotelyAuthenticated != null && isRemotelyAuthenticated.equals("true")) {
				...
				if (requestURL.contains(loginProcessURL)) {
					...
					if ((id == null || "".equals(id)) && (password == null || "".equals(password))) {
						...
						httpRequest.setAttribute("loginMessage", egovMessageSource.getMessage("login.fali.password",request.getLocale()));
						RequestDispatcher dispatcher = httpRequest.getRequestDispatcher(loginURL);
				    ...
				    if(egovLoginConfig.isLock()){
				        try{
				             ...
				             if(mapLockUserInfo != null){		
				                ...
				                if(!sLoginIncorrectCode.equals("E")){
				                    if(sLoginIncorrectCode.equals("L")){
				                        request.setAttribute("loginMessage", egovMessageSource.getMessageArgs("login.fali.loginIncorrect", new Object[] {egovLoginConfig.getLockCount(),request.getLocale()}));
				                    }else if(sLoginIncorrectCode.equals("C")){
				                        request.setAttribute("loginMessage", egovMessageSource.getMessage("login.fali.login",request.getLocale()));
				                    }
							...
							httpRequest.setAttribute("loginMessage", egovMessageSource.getMessage("login.fali.login",request.getLocale()));
							RequestDispatcher dispatcher = httpRequest.getRequestDispatcher(loginURL);
							dispatcher.forward(httpRequest, httpResponse);
				        }
				    }
					...
					try {
						...
						if (loginVO != null && loginVO.getId() != null && !loginVO.getId().equals("")) {
							//세션 로그인
							loginVO.setAuthorCode("ROLE_ADMIN");
							session.setAttribute("loginVO", loginVO);
